...
namespace ManageExcel.UI
{
    public class CriarExcel
    {
        ...
        public static void ExportToExcelOleDb(DataTable table, string connectionString,
                                                          string fileName, bool deleteExistFile, string sheetName)
        {
            // Support for existing file overwrite.
            if (deleteExistFile && File.Exists(fileName))
                File.Delete(fileName);
            ExportToExcelOleDb(table, connectionString, fileName, sheetName);
        }
        ...
        /// <param name="fileName">The Excel file name to export to.</param>
        public static bool ExportToExcelOleDb(DataTable dt, string connectionString, string fileName, string sheetName)
        {
            try
            {
                // Check for null set.
                if (dt.Rows.Count > 0)
                {
                    using (OleDbConnection connection = new OleDbConnection(String.Format(connectionString, fileName)))
                    {
                        ...
                        string strCreateTableStruct = BuildCreateTableCommand(dt, sheetName);
                        ...
                        {
                            command = new OleDbCommand(BuildInsertCommand(dt, rowIndex, sheetName), connection);
                            command.ExecuteNonQuery();
                        }
